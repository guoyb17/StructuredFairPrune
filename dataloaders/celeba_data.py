...
class CelebA(Dataset):
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        img_id = self.df.loc[self.df.index[idx], "Image_Id"]
        img_name = os.path.join(self.root_dir, str(int(img_id.split(".")[0]) % 5), img_id)
...
def celeba_dataloader_score(batch_size, workers, predefined_root_dir='img_align_celeba', csv_file_name='img_align_celeba/list_attr_celeba_modify.txt', fair_type="Male", ctype="Big_Nose"):
...
def celeba_dataloader_score_v2(batch_size, workers, predefined_root_dir='img_align_celeba', csv_file_name='img_align_celeba/list_attr_celeba_modify.txt', fair_type="Male", ctype="Big_Nose", use_val_df=False):
    df = read_celeba_dataset_metainfo_raw(csv_file_name=csv_file_name)
    ...
    return train_dataloader, val_dataloader if not use_val_df else val_df, test_dataloader, train_df

def celeba_test_deploy(batch_size, predefined_root_dir='img_align_celeba', csv_file_name='img_align_celeba/selected.csv', fair_type="Male", ctype="Bags_Under_Eyes"):
...
def celeba_pruning_examples(predefined_root_dir="pruning_examples/celeba_Male_Big_Nose", csv_file_name="pruning_examples/celeba_Male_Big_Nose.csv", minimum=False, ctype="Big_Nose"):
    ...
    for idx in tqdm(range(len(df))):
        img_id = df.loc[df.index[idx], "Image_Id"]
        img_name = os.path.join(predefined_root_dir, str(int(img_id.split(".")[0]) % 5), img_id)
        image = io.imread(img_name)
        if(len(image.shape) < 3):
            image = skimage.color.gray2rgb(image)
